        ...
        <div class="card-body">
            <form action="{{ route('admin.jobs.store') }}" method="POST" enctype="multipart/form-data" id="form-name">
                ...
                <div class="form-group {{ $errors->has('location_id') ? 'has-error' : '' }}">
                    ...
                    <select name="location_id" id="location" class="form-control select2" required>
                        @foreach ($locations as $id => $location)
                            <option value="{{ $id }}" {{ (isset($job) && $job->location ? $job->location->id : old('location_id')) == $id ? 'selected' : '' }}>
                                {{ $location }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('bataslamaran') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="bataslamaran" name="bataslamaran" class="form-control" value="{{ $mitra->bataslamaran }}" min="{{ now()->toDateString('Y-m-d') }}">
                ...
                <div class="form-group {{ $errors->has('categories') ? 'has-error' : '' }}">
                    ...
                    <select name="categories[]" id="categories" class="form-control select2" multiple="multiple">
                        @foreach ($categories as $id => $categories)
                            <option value="{{ $id }}" {{ in_array($id, old('categories', [])) || (isset($job) && $job->categories->contains($id)) ? 'selected' : '' }}>
                                {{ $categories }}</option>
                        @endforeach
                    </select>
